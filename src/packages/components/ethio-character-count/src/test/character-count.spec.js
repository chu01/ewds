...
const characterCountSelector = () =>
  document.querySelector(".ethio-character-count");
const tests = [
...
tests.forEach(({ name, selector: containerSelector }) => {
  describe(`character count component initialized at ${name}`, () => {
    ...
    beforeEach(() => {
      ...
      formGroup = root.querySelector(".ethio-form-group");
      label = root.querySelector(".ethio-label");
      input = root.querySelector(".ethio-character-count__field");
      requirementsMessage = root.querySelector(".ethio-character-count__message");
      statusMessageVisual = root.querySelector(".ethio-character-count__status");
      statusMessageSR = root.querySelector(".ethio-character-count__sr-status");
    });
    ...
    it("hides the requirements hint for screen readers", () => {
      assert.strictEqual(
        requirementsMessage.classList.contains("ethio-sr-only"),
        true,
      );
    });
    ...
    it("creates a screen reader status message on init", () => {
      const srStatus = document.querySelectorAll(
        ".ethio-character-count__sr-status",
      );
