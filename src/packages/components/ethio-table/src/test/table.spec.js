...
const STYLES = fs.readFileSync(
  `${__dirname}/../../../../dist/css/ewds.min.css`,
);

const ASCENDING = "ascending";
const DESCENDING = "descending";
const sortButtonEl = ".ethio-table__header__button";

const tableSelector = () => document.querySelector(".ethio-table");
const tests = [
  ...
  describe(`Sortable Table initialized at ${name}`, () => {
    ...
    beforeEach(() => {
      ...
      ariaLive = body.querySelector(
        ".ethio-table__announcement-region[aria-live='polite']",
      );
    });
    ...
    describe("Sortable column header", () => {
      ...
      it("has sort button with a title that describes what the sort direction will be if clicked", () => {
        ...
        assert.strictEqual(
          firstHeaderButton.classList.contains("ethio-table__header__button"),
          true,
        );
        assert.strictEqual(
      ...
      it("has an SVG which displays the correct icon", () => {
        ...
        const activeSVGNode = sortableHeaders[0].querySelector(
          `.ethio-icon > .${currentSortDirection}`,
        );
        const inactiveSVGNode = sortableHeaders[0].querySelector(
          `.ethio-icon > .${futureSortDirection}`,
        );
        const unsortewdsVGNode = sortableHeaders[0].querySelector(
          ".ethio-icon > .unsorted",
        );
        assert.notEqual(getComputewdstyle(activeSVGNode).fill, "transparent");
        assert.strictEqual(
          getComputewdstyle(inactiveSVGNode).fill,
          "transparent",
        );
        assert.strictEqual(
          getComputewdstyle(unsortewdsVGNode).fill,
          "transparent",
        );
      });
    });
