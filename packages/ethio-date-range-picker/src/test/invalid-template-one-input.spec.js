...
tests.forEach(({ name, selector: containerSelector }) => {
  describe(`Date range picker without second date picker initialized at ${name}`, () => {
    ...
    it('should throw an error when a date range picker without two "ethio-date-picker" elements', () => {
      assert.throws(() => DateRangePicker.on(containerSelector()), {
        message:
          ".ethio-date-range-picker is missing second '.ethio-date-picker' element",
      });
    });
  });
});
